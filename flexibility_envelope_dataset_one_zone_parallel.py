...
POWER_BOUNDS_DIR = os.path.join(BASE_DIR, "data2/power_bounds/")
FLEX_ENV_DIR = os.path.join(BASE_DIR, "data2/flex_env/")
FLEX_IMG_DIR = os.path.join(BASE_DIR, "data2/flex_env_images/")
...
def process_building_climate(args):
    ...
    try:
        ...
        print(f"✅ Done: {building_id} | Climate {climate_id}")

    except Exception as e:
        print(f"⚠️ Skipped {building_id} (climate {climate_id}) due to error:\n{e}")
        return (building_id, climate_id, "error")

    return (building_id, climate_id, "success")


# =====================================================
# MAIN PIPELINE — JOBLIB PARALLEL + TQDM PROGRESS BAR
# =====================================================

if __name__ == "__main__":
    tasks = [(b, c) for b in BUILDING_IDS for c in CLIMATE_IDS]
    n_cores = min(cpu_count(), len(tasks))
    print(f"\nLaunching {len(tasks)} tasks on {n_cores} CPU cores using joblib...\n")

    results = Parallel(n_jobs=n_cores)(
        delayed(process_building_climate)(task)
        for task in tqdm(tasks, desc="Processing Flexibility Envelopes", ncols=100)
    )
